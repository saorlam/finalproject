...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        names= 'Launch Site', 
        title='Portion of Successful Launches per Site')
        return fig
    else:
        data =  spacex_df[spacex_df['Launch Site']==entered_site].copy()
        data['OutcomeLabel'] = data['class'].map({1: 'Success', 0: 'Failure'})

# Count the outcomes
        counts = data['OutcomeLabel'].value_counts().reset_index()
        counts.columns = ['OutcomeLabel', 'Count']
        fig = px.pie(counts,values = 'Count',names = 'OutcomeLabel', title = f'Launch Outcomes at site {entered_site}')
        return fig
...
def get_scatter_chart(entered_site,payloadval):
    low, high = payloadval
    ...
    if entered_site == 'ALL':
        ...
        data =  filtered_df[filtered_df['Launch Site']==entered_site].copy()
